...
trait WoodyThemeTrait_WoodyProcessCompilers
{
    use WoodyThemeTrait_WoodyProcessTools;
    use WoodyThemeTrait_WoodyProcessGetters;
    ...
    public function formatFocusesData($wrapper, $current_post, $twigPaths)
    {
        ...
        switch ($wrapper['acf_fc_layout']) {
            ...
            break;
            case 'auto_focus':
                $the_items = $this->getAutoFocusData($current_post, $wrapper);
            break;
            case 'auto_focus_sheets':
                if (!empty($wrapper['playlist_conf_id'])) {
                    $the_items = $this->getAutoFocusSheetData($wrapper);
                }
            break;
            case 'auto_focus_topics':
                $the_items = $this->getAutoFocusTopicsData($wrapper);
            break;
            case 'profile_focus':
                $the_items = $this->getProfileFocusData($wrapper);
            break;
        }
        ...
        if (!empty($the_items) && !empty($the_items['items']) && is_array($the_items['items'])) {
            ...
            if ($wrapper['acf_fc_layout'] == 'auto_focus_sheets') {
                $the_items['block_titles'] = $this->getFocusBlockTitles($wrapper, 'focus_block_title_');
            } else {
                $the_items['block_titles'] = $this->getFocusBlockTitles($wrapper);
            }
            ...
            if (!empty($wrapper['focus_block_title_bg_params'])) {
                $the_items['display_block_titles'] = $this->getDisplayOptions($wrapper['focus_block_title_bg_params']);
            }
            if (!empty($wrapper['focus_block_bg_params'])) {
                $the_items['display'] = $this->getDisplayOptions($wrapper['focus_block_bg_params']);
            }
    ...
    {
        // Sheet item
        $data = $this->getManualFocusMinisheetData($wrapper);

        // Block titles
        $data['block_titles'] = $this->getFocusBlockTitles($wrapper);
        $data['block_titles']['display_options'] = $this->getDisplayOptions($wrapper);
    ...
    public function formatListContent($wrapper, $current_post, $twigPaths)
    {
        ...
        if (empty($default_items)) {
            $default_items = $this->getAutoFocusData($current_post, $list_el_wrapper, $paginate, $wrapper['uniqid'], true);
        ...
        if (!empty($form_result['uniqid'])) {
            ...
            $the_items = $this->getAutoFocusData($current_post, $list_el_wrapper, $paginate, $wrapper['uniqid'], false, $default_items_ids, $wrapper['the_list_filters']);
        } else {
            $the_items = $this->getAutoFocusData($current_post, $list_el_wrapper, $paginate, $wrapper['uniqid'], false);
        }
        ...
        $the_list = $this->countFocusResults($the_items, $the_list);

        // Récupère la donnée des filtres de base
        if (!empty($wrapper['the_list_filters'])) {
            $the_list['filters'] = $this->getListFilters($wrapper['the_list_filters'], $list_el_wrapper, $default_items);
            // Si on a trouvé un filtre de carte, on remplit le tableau the_map
            if (isset($the_list['filters']['the_map'])) {
                $map_items = $this->getAutoFocusData($current_post, $list_el_wrapper, $paginate, $wrapper['uniqid'], true, $default_items_ids, $wrapper['the_list_filters']);
    ...
    public function formatPageTeaser($context, $custom_post_id = null)
    {
        ...
        if (!empty($page_teaser['page_teaser_img']) && is_array($page_teaser['page_teaser_img'])) {
            $page_teaser['page_teaser_img']['attachment_more_data'] = (!empty($page_teaser['page_teaser_img']['ID'])) ? $this->getAttachmentMoreData($page_teaser['page_teaser_img']['ID']) : [];
        }

        $page_teaser['page_teaser_pretitle'] = (!empty($page_teaser['page_teaser_pretitle'])) ? $this->replacePattern($page_teaser['page_teaser_pretitle'], $context['post_id']) : '';
        $page_teaser['page_teaser_subtitle'] = (!empty($page_teaser['page_teaser_subtitle'])) ? $this->replacePattern($page_teaser['page_teaser_subtitle'], $context['post_id']) : '';
        $page_teaser['page_teaser_desc'] = (!empty($page_teaser['page_teaser_desc'])) ? $this->replacePattern($page_teaser['page_teaser_desc'], $context['post_id']) : '';
    ...
    public function formatPageHero($context, $custom_post_id = null)
    {
        ...
        if (!empty($page_hero['page_heading_media_type']) && ($page_hero['page_heading_media_type'] == 'movie' && !empty($page_hero['page_heading_movie']) || ($page_hero['page_heading_media_type'] == 'img' && !empty($page_hero['page_heading_img'])))) {
            ...
            if (!empty($page_hero['page_heading_img']) && is_array($page_hero['page_heading_img'])) {
                $page_hero['page_heading_img']['attachment_more_data'] = (!empty($page_hero['page_heading_img']['ID'])) ? $this->getAttachmentMoreData($page_hero['page_heading_img']['ID']) : [];
            }
            ...
            $page_hero['title'] = (!empty($page_hero['title'])) ? $this->replacePattern($page_hero['title'], $context['post_id']) : '';
            $page_hero['pretitle'] = (!empty($page_hero['pretitle'])) ? $this->replacePattern($page_hero['pretitle'], $context['post_id']) : '';
            $page_hero['subtitle'] = (!empty($page_hero['subtitle'])) ? $this->replacePattern($page_hero['subtitle'], $context['post_id']) : '';
            $page_hero['description'] = (!empty($page_hero['description'])) ? $this->replacePattern($page_hero['description'], $context['post_id']) : '';
