...
trait WoodyThemeTrait_WoodyProcessGetters
{
    use WoodyThemeTrait_WoodyProcessQuery;
    ...
    {
        $the_items = [];
        $query_result = $this->processWoodyQuery($current_post, $wrapper, $paginate, $uniqid, $ingore_maxnum, $posts_in, $filters);
    ...
    public function getPagePreview($wrapper, $item, $clickable = true)
    {
        ...
        if (!empty(get_field('focus_title', $item->ID))) {
            $data['title'] = $this->replacePattern(get_field('focus_title', $item->ID), $item->ID);
        } elseif (!empty(get_the_title($item->ID))) {
            $data['title'] = $this->replacePattern(get_the_title($item->ID), $item->ID);
        }
        ...
        if (!empty($wrapper) && !empty($wrapper['display_elements']) && is_array($wrapper['display_elements'])) {
            if (in_array('pretitle', $wrapper['display_elements'])) {
                $data['pretitle'] = $this->replacePattern($this->getFieldAndFallback($item, 'focus_pretitle', get_field('page_heading_heading', $item->ID), 'pretitle', $item, 'field_5b87f20257a1d'), $item->ID);
            }
            if (in_array('subtitle', $wrapper['display_elements'])) {
                $data['subtitle'] = $this->replacePattern($this->getFieldAndFallback($item, 'focus_subtitle', get_field('page_heading_heading', $item->ID), 'subtitle', $item, 'field_5b87f23b57a1e'), $item->ID);
            }
            ...
            if (in_array('description', $wrapper['display_elements'])) {
                $data['description'] = $this->replacePattern($this->getFieldAndFallback($item, 'focus_description', $item, 'field_5b2bbbfaec6b2'), $item->ID);
            }
            ...
            if (in_array('linked_profil', $wrapper['display_elements'])) {
                ...
                if ($fields_profil['img']) {
                    ...
                    $data['profil']['img']['attachment_more_data'] = $this->getAttachmentMoreData($fields_profil['img']['ID']);
                }
        ...
        if ($clickable) {
            $data['link']['link_label'] = $this->replacePattern($this->getFieldAndFallBack($item, 'focus_button_title', $item), $item->ID);
        ...
        if (!empty($wrapper['display_img'])) {
            $data['img'] = $this->getFieldAndFallback($item, 'focus_img', $item, 'field_5b0e5ddfd4b1b');
            if (empty($data['img'])) {
                $video = $this->getFieldAndFallback($item, 'field_5b0e5df0d4b1c', $item);
                $data['img'] = !empty($video) ? $video['movie_poster_file'] : '';
            }
            if (!empty($data['img'])) {
                $data['img']['attachment_more_data'] = $this->getAttachmentMoreData($data['img']['ID']);
    ...
    public function getCustomPreview($item, $wrapper = null)
    {
        ...
        if ($item['media_type'] == 'img' && !empty($item['img'])) {
            $data['img'] = $item['img'];
            $data['img']['attachment_more_data'] = $this->getAttachmentMoreData($item['img']['ID']);
    ...
    public function getTouristicSheetPreview($wrapper = null, $item, $deal_index = 0)
    {
        ...
        $data = [];
        $sheet = $this->getTouristicSheetData($item, $current_lang);
            ...
            foreach ($filter_wrapper['list_filters'] as $key => $filter) {
                switch ($filter['list_filter_type']) {
                    ...
                    case 'duration':
                        ...
                        $return[$key]['minmax']['max'] = $this->getMinMaxWoodyFieldValues($default_items['wp_query']->query_vars, $field);
                        $return[$key]['minmax']['min'] = $this->getMinMaxWoodyFieldValues($default_items['wp_query']->query_vars, $field, 'min');
    ...
    public function getManualFocusMiniSheetData($wrapper)
    {
        ...
        $current_lang = pll_current_language();
        $sheet = $this->getTouristicSheetData($post, $current_lang);
